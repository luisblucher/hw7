...
firebase.auth().onAuthStateChanged(async function(user) {
    ...
    if (user) {
        ...
        document.querySelector('.sign-in-or-sign-out').innerHTML =`
            ...
            <div ><a href="#" class="sign-out text-pink-500 underline">Sign out</a></div>
            </div>
        `
        ...
        let apiKey = '03e8e24206878cf11fcfb268243f73df'
        ...
        for (let i=0; i<movies.length; i++) {
            let movie = movies[i]
            let docRef = await db.collection('watched').doc(`${movie.id}_${user.uid}`).get()
        ...
        document.querySelector('.movies').insertAdjacentHTML('beforeend', `
            <div class="w-1/5 p-4 movie-${movie.id}_${user.uid} ${opacityClass}">
            <img src="https://image.tmdb.org/t/p/w500${movie.poster_path}" class="w-full">
            <a href="#" class="watched-button block text-center text-white bg-green-500 mt-4 px-4 py-2 rounded">I've watched this!</a>
            </div>
        `)
    
        document.querySelector(`.movie-${movie.id}_${user.uid}`).addEventListener('click', async function(event) {
            event.preventDefault()
            let movieElement = document.querySelector(`.movie-${movie.id}_${user.uid}`)
            movieElement.classList.add('opacity-20')
            await db.collection('watched').doc(`${movie.id}_${user.uid}`).set({
            })
        }) 
        }
